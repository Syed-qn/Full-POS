...
Renders cart state from DB without touching the LLM. Functions here NEVER
import from engine.py to avoid circular imports — shared helpers reimplemented
inline with identical logic.
"""
...
def _note_suffix(it) -> str:
    """Render an order item's special note suffix (identical logic to engine._note_suffix)."""
...
def _render_cart_tail(items, order) -> str:
    """Render the \\n\\n🛒 cart tail for the ordering phase."""
    if not items:
...
async def _render_confirmation_body(session: AsyncSession, items, order) -> str:
    """Render the full order summary text body for awaiting_confirmation phase.

    Mirrors the body built by engine._send_order_summary (without buttons).
    W5 hook: add wallet/coupon redeem_block below the TODO comment.
    """
    ...
    # TODO(W5): wallet/coupon composition — insert redeem_block here
    return (
        f"Order summary:\n{item_lines}\n\n"
